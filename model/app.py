...
conn = sqlite3.connect(os.path.join(os.path.dirname(__file__), 'user_credentials.db'))
cursor = conn.cursor()
...
def predict_price(location, total_sqft, bath, bhk):
    # Load trained model
    model_path = os.path.join(os.path.dirname(__file__), "house_price_prediction.pickle")
...
def prediction_page():
    ...
    # Load Image
    image_path = os.path.join(os.path.dirname(__file__), "..", "images", "house.jpg")
    ...
    if st.button("Predict"):
        ...
        predicted_price_pkr = predicted_price * 100000  # Convert from lakhs to PKR
